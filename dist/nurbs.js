(function (global, factory) {
  ...
  (global = global || self, global.nurbs = factory());
  ...
  var isNdarrayLike = function (arr) {
    if (!arr) return false;
    return arr.data !== undefined && Array.isArray(arr.shape) && arr.offset !== undefined && arr.stride !== undefined;
  };
  ...
  var cacheKey = function (nurbs, debug, checkBounds, pointType, weightType, knotType) {
    ...
    for (d = 0; d < nurbs.splineDimension; d++) {
      ...
      degreeParts.push('Deg' + nurbs.degree[d] + (hasKnots ? '' : 'Uniform') + capitalize(nurbs.boundary[d]));
    }
    var parts = [[hasAnyKnots ? 'NU' : '', nurbs.weights ? 'RBS' : 'BS'].join('') + nurbs.dimension + 'D', degreeParts.join('_')];
        ...
        for (i = -degree[d] + 1; i <= degree[d]; i++) {
          if (boundary[d] === 'closed') {
            if (i < 0) {
              ...
              // EDIT THIS SECTION
              line('var ' + knotVar([d, i + degree[d] - 1]) + ' = ' + ternary(knotIndex(d) + ' < ' + -i, knotAccessor([d, 0]) + ' + ' + knotAccessor([d, [sizeVar(d), knotIndex(d), i]]) + ' - ' + knotAccessor([d, [sizeVar(d)]]), knotAccessor([d, [knotIndex(d), i]])) + ';');
            } else if (i > 0) {
              line('var ' + knotVar([d, i + degree[d] - 1]) + ' = ' + ternary(knotIndex(d) + ' + ' + i + ' > ' + sizeVar(d),
              // knotAccessor([d, sizeVar(d)]) + ' + ' + knotAccessor([d, i]) + ' - ' + knotAccessor([d, 0]),
              knotAccessor([d, sizeVar(d)]) + ' + ' + knotAccessor([d, i + ' + ' + knotIndex(d) + ' - ' + sizeVar(d)]) + ' - ' + knotAccessor([d, 0]), knotAccessor([d, [knotIndex(d), i]])) + ';');
            } else {
              line('var ' + knotVar([d, i + degree[d] - 1]) + ' = ' + knotAccessor([d, [knotIndex(d), i]]) + ';');
            }
          } else {
  ...
  function parseNURBS(points, degree, knots, weights, boundary, opts) {
    ...
    // Create evaluator
    //
    var newCacheKey = cacheKey(this, this.debug, this.checkBounds, pointType, weightType, knotType);
